...
async function loadHomeData() {
  const [categoryResult, flashResult, recommendedResult] = await Promise.all([
    env.DB.prepare(
      "SELECT id, name, slug FROM Category ORDER BY name ASC LIMIT 6"
    ).all<CategoryRow>(),
    env.DB.prepare(
      "SELECT id, name, slug, price, discount, images, isFlashSale FROM Product WHERE isFlashSale = 1 ORDER BY createdAt DESC LIMIT 6"
    ).all<HomeProduct>(),
    env.DB.prepare(
      "SELECT id, name, slug, price, discount, images, isFlashSale FROM Product ORDER BY createdAt DESC LIMIT 12"
    ).all<HomeProduct>(),
  ]);

  return {
    categories: categoryResult.results ?? [],
    flashDeals: flashResult.results ?? [],
    recommended: recommendedResult.results ?? [],
  };
}
    ...
    <div className="min-h-screen font-sans text-zinc-950 dark:text-zinc-50">
      ...
      <main className="mx-auto w-full max-w-6xl px-4 py-6">
          ...
          <div className="relative overflow-hidden rounded-3xl border border-black/10 bg-gradient-to-br from-orange-500 via-amber-400 to-yellow-200 p-6 text-zinc-950 shadow-sm dark:border-white/10 lg:col-span-8">
            <div className="max-w-md">
              ...
              <h1 className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">
                Big savings on trending essentials
              </h1>
              <p className="mt-2 text-sm text-zinc-800/90">
                Browse flash sales, discover new brands, and checkout fast.
              </p>
            ...
            <div className="mt-4 grid gap-2">
              {categories.map((c) => (
                <Link
                  ...
                  className="group flex items-center justify-between rounded-2xl border border-black/10 bg-zinc-50 px-4 py-3 text-left transition hover:bg-zinc-100 dark:border-white/10 dark:bg-black dark:hover:bg-white/5"
                >
                  <div>
              ...
              {categories.length === 0 ? (
                <div className="rounded-2xl bg-zinc-50 px-4 py-3 text-xs text-zinc-500 dark:bg-black dark:text-zinc-400">
        ...
        <section className="mt-8">
          ...
          {flashDeals.length === 0 ? (
            <div className="mt-4 rounded-3xl border border-black/10 bg-white p-6 text-sm text-zinc-600 dark:border-white/10 dark:bg-zinc-950 dark:text-zinc-400">
              No flash deals are available yet.
            </div>
          ) : (
        ...
        <section className="mt-10">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-600 dark:text-zinc-400">
              Popular picks, refreshed daily.
            </p>
          </div>

          {recommended.length === 0 ? (
            <div className="mt-4 rounded-3xl border border-black/10 bg-white p-6 text-sm text-zinc-600 dark:border-white/10 dark:bg-zinc-950 dark:text-zinc-400">
              Products will appear here once the store is stocked.
            </div>
          ) : (
